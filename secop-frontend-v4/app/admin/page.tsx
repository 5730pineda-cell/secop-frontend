...
// ---------- MODAL NUEVO CLIENTE (con prefill) ----------
...
export default function AdminPage() {
  ...
  // Estados para edición de fechas de etapa en solicitudes
  ...
  // Estado para prellenar cliente desde solicitud
  ...
  return (
    <div className="min-h-screen bg-[#0a0c10] text-white font-sans">
      ...
      <div className="max-w-[1400px] mx-auto p-6">
        ...
        {/* TAB PROCESOS (existente, con comentarios, etapas, etc.) */}
        {tab === "procesos" && (
                  ...
                  <div key={sol.id} className="bg-[#15181f] border border-[#252932] rounded-xl p-5">
                    ...
                    <div className="flex justify-between items-start flex-wrap gap-2 mb-3">
                      ...
                      <div className="flex gap-2">
                        <select
                          value={sol.estado}
                          onChange={e => actualizarEstadoSolicitud(sol.id, e.target.value)}
                          className="text-xs bg-[#1c2028] rounded px-2 py-1"
                        >
                        ...
                        <button
                          onClick={() => {
                            setPrefillCliente({
                              nombre: sol.empresa,
                              descripcion_negocio: `Solicitud de acompañamiento: ${sol.numero_proceso}`,
                              email_destinatario: sol.empresa.includes('@') ? sol.empresa : undefined
                            })
                            setShowNuevoCliente(true)
                          }}
                          className="text-xs bg-[#3b82f6] px-2 py-1 rounded"
                        >
                          Crear cliente
                        </button>
                      </div>
                    ...
                    {/* Seguimiento de etapas con nombre personalizable */}
                    <div className="border-t border-[#252932] pt-3 mb-3">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <div className="flex items-center gap-2">
                          <input
                            type="text"
                            value={sol.etapa_nombre || ETAPAS[sol.etapa_actual || 0]}
                            onChange={(e) => actualizarNombreEtapaSolicitud(sol.id, e.target.value)}
                            className="text-[10px] bg-[#1c2028] rounded px-2 py-1 text-[#f59e0b] w-36 text-center"
                            placeholder="Nombre personalizado"
                          />
                          <span className="text-[9px] text-[#525a68]">(estándar: {ETAPAS[sol.etapa_actual || 0]})</span>
                        </div>
                      </div>
                      <TimelineAdmin procesoId={sol.id} etapa={sol.etapa_actual || 0} onUpdate={async (id, etapaNueva) => {
                      ...
                      <div className="mt-2 flex gap-2">
                        <button
                          onClick={() => setEditarFechasEtapa({ solicitudId: sol.id, etapa: sol.etapa_actual || 0 })}
                          className="text-xs bg-[#1c2028] px-2 py-1 rounded"
                        >
                          <Calendar size={10} className="inline mr-1" /> Fecha etapa
                        </button>
                      </div>
                    ...
                    <div className="border-t border-[#252932] pt-3">
                      ...
                      <div className="flex gap-1">
                        <input
                          ...
                          className="flex-1 text-xs bg-[#1c2028] p-2 rounded"
                          value={nuevoComentarioSolicitud[sol.id] || ""}
                          onChange={e => setNuevoComentarioSolicitud({...nuevoComentarioSolicitud, [sol.id]: e.target.value})}
                          onKeyDown={e => { if (e.key === 'Enter') enviarComentarioSolicitud(sol.id, sol.cliente_id) }}
                        />
